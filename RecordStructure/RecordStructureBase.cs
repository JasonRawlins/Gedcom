...
public class RecordStructureBase
{
    ...
    public void SetRecord(Record record) => Record = record;

    public static T CreateRecordStructure<T>(RecordStructureBase recordStructure, string tag) where T : RecordStructureBase, new()
    {
        return CreateRecordStructures<T>(recordStructure, tag).First();
    }
    public static List<T>? CreateRecordStructures<T>(RecordStructureBase recordStructure, string tag) where T : RecordStructureBase, new()
    {
        var records = recordStructure.Record.Records.Where(r => r.Tag.Equals(tag))
            .Select(r =>
            {
    ...
    protected List<T> List<T>(string tag) where T : RecordStructureBase, new ()
    {
        var records = List(tag);
        if (records != null)
        {
